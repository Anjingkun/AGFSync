...
class AestheticScorer(Scorer):
    ...
    def _calculate_score(self):
        ...
        for img in self.images:
            image = torch.from_numpy(self.clip_processor(img)['pixel_values'][0]).unsqueeze(0).to(self.clip_model.device)
            with torch.no_grad():
                image_features = self.clip_model.get_image_features(image)

            im_emb_arr = normalized(image_features.cpu().detach().numpy())
            prediction = self.model(torch.from_numpy(im_emb_arr).to(self.clip_model.device))
